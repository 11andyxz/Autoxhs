...
import { tailorSpecFromForm } from "@/lib/job-hunter/tailorMode";
...
export async function POST(req: NextRequest) {
  ...
  const spec = tailorSpecFromForm(form);
  ...
  try {
    ...
    const [data, tailoredHtml] = await Promise.all([
      generateTailoredResume(resumeText, jdText, spec),
      tailorResumeHtmlToJd(resumeHtml, jdText, spec),
    ]);
