...
import { getCoachSr, rateCoach } from "@/lib/job-hunter/interview/repo";
...
/** 讲解复习:拖动「我理解了 X%」→ 折算成 SM-2 quality → 排下次复习(遗忘曲线)。 */
export async function POST(req: NextRequest) {
  if (tooMany(req)) return rateLimited();

  let body: { skillId?: unknown; pct?: unknown };
  try {
    body = await req.json();
  } catch {
    return bad("请求格式有误。");
  ...
  try {
    const cur = await getCoachSr(skillId);
    ...
    await rateCoach(skillId, sr, pct);
